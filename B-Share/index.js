...
app.post('/library',function(req,res){
  ...
  if(books instanceof Array){
    ...
    for(var i=0;i<length;i++){
      db.all('insert into books (book,available,use) values(?,?,?);',[books[i],1,'false'],function(err){
      });
    }
  } else {
      db.all('insert into books (book,available,use) values(?,?,?);',[books,1,'false'],function(err){
      });
  }
  res.end('Success');
});
...
app.get('/all',function(req,res){
  db.all("select needReturn.user,needReturn.book,user.email from needReturn join user on (needReturn.user=user.user) where needReturn.isOK='no'",function(err,data){
    res.send(data);
  });
});
